...
- `ThemeDirectory.jsx`（`'use client'`）：主题目录组件，支持两种模式：
    ...
    - 统一行为：读取 `useTheme()` 的 `themes`/`activeId`/`setActiveTheme`，维护 `view` 状态（`list`/`grid`）；底部固定栏左对齐提供 List/Grid 两种视图切换胶囊按钮（复刻 TUX Work 页底部导航栏）；List 视图以主题行展示名称、标签和缩略图；切换视图时使用 GSAP Flip 对带 `data-flip-id` 的图片做跨视图位移动画，并带锁防止快速连续切换；列表入场只对行项目做轻量 `y:64→0` 与 `opacity` stagger，避免父子双层大位移造成字体抖动；当前主题以左侧常驻占位圆点或网格边框标识。
- `ThemeDirectory.module.scss`：**要点** `.panel` 为 `position:fixed; inset:0; z-index:2000; background:#fff` 全屏 flex 列（head/内容/foot），`.page` 模式下为 `height:100svh`，使内容区独立滚动；`.foot` 为带顶部分隔线和高层级的底部导航栏，始终显示切换胶囊；`.viewSwitch` 为圆角胶囊按钮组；`.row` 为稳定高度的纵向主题行，使用 `will-change: transform, opacity` 和 `translateZ(0)` 承接 GSAP 轻位移动画；`.part1` 固定最小高度，`.dot` 常驻占位并由 active 控制显隐，避免主题切换时文字横向跳动；hover 效果限制在 `@media (hover:hover) and (pointer:fine)`，通过 `::before` 伪元素实现黑色背景条自底部 `scaleY(0→1)` 展开；`.trait` 顶部 1px 分隔线并仅声明 `will-change: transform`；缩略图固定 64px。
