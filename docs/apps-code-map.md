...
│  ├─ debug/                    开发调试工具（FloatingLines 可视化参数面板等）
...
> 静态资源按主题分目录：`public/photos/<主题id>/`（如 `public/photos/default/img1~33.webp`、`public/photos/fengjing-jianzhi/风景剪纸_1~17.webp`、`public/photos/水彩_壁纸风景/水彩壁纸风景_1~14.webp`）。
...
| 文件            | 职责                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AppLayout.jsx` | 持有滚动容器 `ref`；挂载固定全屏 `BackgroundLayer` 作为 2D 背景；`next/dynamic`（ssr:false）挂载透明全屏 Scene，并传入 `eventSource`/`eventPrefix="client"`；以 `ThemeProvider` 包裹全站；整个应用壳由 `PageTransitionProvider` 托管，Header、Footer 与目录页统一调用 `navigateWithTransition`，待黑幕完全覆盖后才执行路由跳转；Footer 视图切换仅在 `/` 与 `/paper` 显示，主题目录 `/directory` 隐藏；不再维护目录面板 state |

### 调试工具 `src/components/debug/`

| 文件                                                              | 职责                                                                                                                                                                                                                                                        |
| ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `FloatingLinesDebugger.jsx` + `FloatingLinesDebugger.module.scss` | FloatingLines 动效系统可视化调节面板：URL query `?debug=floating-lines` 触发，实时调节 2D 背景漂浮线条的颜色、波浪数量/间距、位置、动画速度、鼠标交互、视差与混合模式，并导出 JSON 配置用于替换 `FLOATING_LINES_PRESET`；仅用于开发调试，不影响生产默认渲染 |

### 背景系统 `src/components/background/`

| 文件                                                                          | 职责                                                                                                                                                                                                                                                                                                               |
| ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `BackgroundLayer.jsx` + `BackgroundLayer.module.scss` + `FloatingLines2D.jsx` | 固定全屏 2D 背景层：读取当前主题与路由，输出塔式、纸片、目录页差异化的 CSS 光场、结构线、网格、纹理和首页 React Bits 风格漂浮线条；内部维护主题调色板映射并通过 CSS 变量与 shader 渐变驱动背景层；`pointer-events:none` 且 `aria-hidden`，位于透明 3D Canvas 下方；调试模式下接收 `FloatingLinesDebugger` 实时配置 |
...
| 文件                                                | 职责                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PageTransition.jsx` + `PageTransition.module.scss` | 全局页面过渡动画层：以 `idle → leaving → waiting-route → entering` 阶段控制 GSAP Timeline，实现「旧页面轻上滑 → 黑色遮罩完全覆盖 → 执行路由跳转 → 新页面在黑幕下提交 → 遮罩离场」的 wipe 过渡；Header、Footer 与目录页经 `usePageTransition()` 的 `navigateWithTransition` 触发，避免新页面在动画开始前闪现；通过双 `requestAnimationFrame` 等待新页面首帧提交再释放黑幕，并为浏览器前进、后退保留路由变化兜底处理；支持 `prefers-reduced-motion` |
...
| `components/theme/ThemeDirectory.jsx` + `ThemeDirectory.module.scss` | 主题目录组件，支持 `mode='overlay'`（遗留全屏覆盖层，经 `createPortal` 渲染到 `body`）与 `mode='page'`（独立页面 `/directory`）两种模式；标题栏 + 内容区 + 底部三段式 flex 布局，内容区独立滚动，底栏常驻视口底部；标题「主题目录」为可点击返回入口；底部固定栏左对齐放置 List/Grid 视图切换胶囊，复刻 TUX Work 页底部导航栏；支持列表与网格两种视图切换，切换时使用 GSAP Flip 对同一图片做跨视图位移动画；List View 使用名称区、元信息区（首个主题标签 + 导入日期值/操作）与缩略图区的稳定栅格，导入日期展示态不显示标题文本；入场改为行元素单层 `y:64→0` + `opacity` stagger，避免列表容器与行元素双层大位移导致字体抖动；`.dot` 常驻占位，active 仅控制显隐，避免主题切换时名称横向位移；首屏列表缩略图 eager 加载并 async 解码；全动画链路支持 `prefers-reduced-motion` 直接显示最终态；详见 [主题目录模块说明](./主题系统/主题目录-模块说明.md) |

### 数据 `src/data/`

| 文件          | 职责                                                                                                                                                                                                                          |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `themes.js`   | 预设主题注册表 `presetThemes`：每项 `{ id, name, cover?, tags?, importedAt?, images:[{url}] }`；当前注册 `default`（复用 `images.js`）、`fengjing-jianzhi`（17 张 WebP）与 `shuicai-bizhi-fengjing`（14 张 WebP）三个正式主题 |
...
- 主题日期：`src/data/themes.js` 提供默认 `importedAt`；`src/context/ThemeContext.jsx` 维护本地覆盖值；`src/components/theme/ThemeDirectory.jsx` 与样式模块负责展示、修改和恢复默认日期。
